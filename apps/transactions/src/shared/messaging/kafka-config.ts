...
// One shared raw kafkajs client for anything that needs a consumer/producer outside the
// @nestjs/microservices wrapper (e.g. the SSE fan-out consumer, which needs a unique groupId
// per instance that the wrapper doesn't support). Kafka() itself is just connection config,
// not a socket, so sharing it gives future consumers one place to get a client from.
export function getKafkaClient(): Kafka {
  client ??= new Kafka(getKafkaClientOptions());
  return client;
}
